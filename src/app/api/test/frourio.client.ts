...
export const fc = (option?: FrourioClientOption) => ({
  ...
  $build(req?: {
    ...
    return [{ dir: '/api/test' }, () => fc(option).$get(req)];
  },
  ...methods(option),
});
...
export const $fc = (option?: FrourioClientOption) => ({
  ...
  async $get(
    req?: Parameters<ReturnType<typeof methods>['$get']>[0]
  ...
  async $post(
    req: Parameters<ReturnType<typeof methods>['$post']>[0]
  ): Promise<z.infer<(typeof frourioSpec.post.res)[200]['body']>> {
...
const $url = (option?: FrourioClientOption) => ({
  get():
    ...
    return { isValid: true, data: `${option?.baseURL ?? ''}/api/test` };
  },
  post():
    | { isValid: true; data: string; reason?: undefined }
    | { isValid: false; data?: undefined; reason: z.ZodError } {
...
const methods = (option?: FrourioClientOption) => ({
  async $get(req?: {
    ...
    | {
        ...
        data: {
          status: 200;
          headers?: undefined;
          body: z.infer<(typeof frourioSpec.get.res)[200]['body']>;
        };
        failure?: undefined;
        raw: Response;
        reason?: undefined;
  ...
  > {
    ...
    const result: { success: true; res: Response } | { success: false; error: unknown } =
      await fetchFn(url.data, {
        method: 'GET',
        ...option?.init,
        ...req?.init,
    ...
    switch (result.res.status) {
      case 200: {
        const resBody: { success: true; data: unknown } | { success: false; error: unknown } =
          await result.res
            .json()
  ...
  async $post(req: {
    body: z.infer<typeof frourioSpec.post.body>;
    init?: RequestInit;
  }): Promise<
    | {
        ok: true;
        ...
        data: {
          status: 200;
          headers?: undefined;
          body: z.infer<(typeof frourioSpec.post.res)[200]['body']>;
        };
        failure?: undefined;
        raw: Response;
        reason?: undefined;
    ...
    | {
        ...
        failure: {
          status: 404;
          headers?: undefined;
          body: z.infer<(typeof frourioSpec.post.res)[404]['body']>;
        };
        raw: Response;
        reason?: undefined;
        error?: undefined;
  ...
  > {
    ...
    switch (result.res.status) {
      ...
      case 404: {
        ...
        const body = frourioSpec.post.res[404].body.safeParse(resBody.data);

        if (!body.success)
          return { ok: false, isValid: false, raw: result.res, reason: body.error };
